...
export default function ProfileHeader({ character, username, isMaster }: ProfileHeaderProps) {
  ...
  // Função auxiliar para resolver o segundo slot de atributo com base na categoria
  const renderCategoryAttribute = (char: ProfileCharacter) => {
    switch (char.category) {
      case 'aluno':
        return { label: 'Ano Escolar', value: `${char.year}º Ano` };
      case 'professor':
        return { label: 'Matéria', value: char.subject };
      case 'npc':
        return { label: 'Ocupação', value: char.occupation };
      case 'monstro':
        return { label: 'Perigo', value: char.dangerLevel };
      default:
        return { label: 'Tipo', value: 'Desconhecido' };
    }
  };
      ...
      <div className="flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="relative w-40 h-40 rounded-full border-2 border-bege-medio overflow-hidden">
          {character?.image ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-bege-claro text-4xl font-serif">?</div>
          )}
        </div>
        ...
        <div className="flex-1 w-full space-y-4">
          <div>
            <h1 className="text-3xl text-bege-medio tracking-wide font-title uppercase">{displayName}</h1>
            <p className="font-title text-sm text-bege-escuro uppercase tracking-wider">
              {isMaster ? 'Mestre da Campanha' : `${character?.category} — Nível ${character?.level || 1}`}
            </p>
          </div>
          ...
          {/* Grid de Atributos com Lógica do Arquetipo */}
