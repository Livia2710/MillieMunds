...
export function WorldCard({
  ...
  return (
    ...
    <article className="relative mx-auto aspect-[11/15] w-full max-w-60 overflow-hidden rounded-[12px] text-bege-escuro">
      ...
      {image && !isLocked && (
        <Image
          ...
          width={220}
          height={300}
          className="absolute inset-0 h-full w-full object-cover opacity-55 p-[6px] rounded-[14px]"
        />
      )}

      {/* Camada de cor de fundo escurecida: Também recebe o mesmo recuo para não vazar a cor nas quinas transparentes */}
      <div className="absolute inset-0 z-10 bg-roxo-escuro/45 m-[6px] rounded-[14px]" />
      ...
      <div className="relative z-30 flex h-full flex-col items-center justify-center px-6 py-4 text-center">
        ...
        <h3 className="mt-2 w-full break-words font-title text-base uppercase tracking-[0.16em] md:mt-5 md:text-xl">
          {displayName}
        </h3>
