...
const BRANCH_OPTIONS: { value: SkillBranch; label: string }[] = [
  { value: "ativa", label: "Ativa" }, { value: "passiva", label: "Passiva" },
  { value: "reacao", label: "Reação" }, { value: "aprimoramento", label: "Aprimoramento" },
]
...
export default function CriarHabilidadeModal({ isOpen, onClose }: Props) {
  ...
  function handleClose() {
    ...
    setElement("fogo"); setMaxLevel("3"); setRequiredLevel("1"); setError(""); onClose()
  }
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      try {
        await createSkill({ name: name.trim(), description, branch, element, maxLevel: Number(maxLevel), requiredCharacterLevel: Number(requiredLevel) })
        router.refresh()
        handleClose()
      } catch (err: unknown) {
  ...
  return (
    <MillieModal isOpen={isOpen} onClose={handleClose} title="Criar Habilidade">
      <div className="space-y-4">
        <MillieInput label="Nome" placeholder="Ex: Chama Eterna" value={name} onChange={(e) => setName(e.target.value)} />
        <MillieTextarea label="Descrição" placeholder="Descreva o efeito..." rows={3} value={description} onChange={(e) => setDescription(e.target.value)} />
        ...
        <div className="flex items-center gap-3 border px-4 py-3" style={{ borderColor: selectedMeta.color + "40" }}>
          ...
          <div>
            <p className="font-title text-xs uppercase tracking-widest" style={{ color: selectedMeta.color }}>{selectedMeta.label}</p>
        ...
        <div className="flex justify-end gap-3 border-t border-bege-escuro/20 pt-4">
          ...
          <PrimaryButton onClick={handleSubmit} disabled={isPending}>{isPending ? "Criando..." : "Criar Habilidade"}</PrimaryButton>
        </div>
      </div>
    </MillieModal>
  )
