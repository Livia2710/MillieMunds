...
function getAttributePoints(rank: string) {
  switch (rank) {
    case "C": return 35;
    case "B": return 45;
    case "A": return 55;
...
function getStudentRank(year: number) {
  switch (year) {
    case 1: return "D";
    case 2: return "C";
    case 3: return "B";
...
export default function CriarPersonagemModal({ isOpen, onClose }: Props) {
  ...
  const [manualRank, setManualRank] = useState("D");
  ...
  const startLevel = category === "aluno"
    ...
    : Math.max(1, Number(manualLevel) || 1);

  const raceIsCorrupted = races.find((r) => r.id === raceId)?.isCorrupted ?? false;

  const currentRank = category === "aluno"
    ? getStudentRank(Number(year))
    : category === "professor" || category === "npc"
      ? manualRank
      : calcCurrentRank(startLevel, baseRank, raceIsCorrupted);

  const totalPoints = getAttributePoints(currentRank);
  const usedPoints = ATTR_KEYS.reduce((total, key) => total + attrs[key], 0);
  const remainingPoints = totalPoints - usedPoints;

  const computedDangerLevel = category === "monstro"
    ? calcDangerLevelByLevel(startLevel)
    : undefined;
  ...
  useEffect(() => {
    ...
    const otherSum = ATTR_KEYS
      .filter((k) => k !== key)
      .reduce((sum, k) => sum + attrs[k], 0);

    const maxAllowed = totalPoints - otherSum;
  ...
  function handleSubmit() {
    ...
    if (!raceId) return setError("Selecione uma raça.");
    if (remainingPoints !== 0) return setError("Distribua todos os pontos antes de continuar.");
    ...
    startTransition(async () => {
      try {
        await createCharacter({
          ...
          dangerLevel: category === "monstro" ? computedDangerLevel : undefined,
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="border border-bege-escuro/20 bg-roxo-escuro/40 px-3 py-2.5">
              <span className="font-title text-xs capitalize text-bege-medio/60">
                {element || "—"}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="border border-bege-escuro/20 bg-roxo-escuro/40 px-3 py-2.5 flex items-center justify-between">
              <span className="font-title text-xs text-bege-medio">
                {baseRank}
              </span>
        ...
        {category === "aluno" && (
          <>
            ...
            <div className="border border-bege-escuro/20 bg-roxo-escuro/40 px-3 py-2.5">
              <div className="font-title text-[10px] uppercase tracking-widest text-bege-escuro/60">
                Rank Atual
              </div>
              <div className="font-title text-xl text-bege-medio">
                {currentRank}
              </div>
            </div>
          </>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <MillieSelect
                label="Rank Atual"
                options={RANK_OPTIONS.map((rank) => ({ value: rank, label: rank }))}
                value={manualRank}
        ...
        <div className="pt-2 border-t border-bege-escuro/20 space-y-3">
          ...
          {ATTR_KEYS.map((key) => (
            <div key={key} className="flex items-center gap-3">
              ...
              <div className="flex-1 h-0.5 bg-bege-escuro/10 relative">
                <div
                  ...
                  style={{
                    width: `${Math.min(100, (attrs[key] / Math.max(1, totalPoints)) * 100)}%`,
                  }}
                />
              </div>

              <span className="font-title text-[10px] text-bege-escuro/40 w-8 text-right">
                {attrs[key]}
              </span>
            </div>
          ))}
            ...
            <div className="bg-roxo-escuro/30 border border-bege-escuro/20 p-3 text-center">
              <div className="font-title text-[10px] text-bege-escuro/60 uppercase tracking-widest mb-1">
                Pontos de Vida
              </div>
              <div className="font-title text-xl text-bege-medio">
                {pv}
              </div>
              <div className="font-title text-[10px] text-bege-escuro/40">
                10 + (Vigor {attrs.vigor} × 2)
              </div>
            </div>
